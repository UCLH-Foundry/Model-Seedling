...
def compare_distributions(reference_df,new_df,cat_col):
    ...
    '''
    categorical_columns_dict = {list(reference_df.columns).index(i):None for i in cat_col}
    cd = TabularDrift(reference_df.values, p_val=.05,categories_per_feature=categorical_columns_dict)
...
def gen_categorical_metrics(reference_df,new_df,col):
    ...
    metrics_dict['most_common_category_freq_new'] = str(most_frequent_category_new[0])
    ...
    metrics_dict['most_common_category_freq_ref'] = str(most_frequent_category_ref[0])
    return metrics_dict
...
def main(args):
    ...
    for id,col in enumerate(reference_df.columns):
        ...
        if id in cat_cols_name:
            fig = cat_bar_plot(reference_df,new_df,col,int(drift_pred['is_drift'][id]))
            mlflow.log_figure(fig, f'{col}_frequency.png')
            feature_metrics = gen_categorical_metrics(reference_df,new_df,col)
        else:
